...
void setup() { 
  Serial.begin(115200);
  Wire.begin();
    while (!huskylens.begin(Wire)) //checking if HUSKYLENS is connected with I2C
    {
        delay(100);
    }
...
void loop() {
  ...
  if (!huskylens.request()){ //checking if the ball is detected and has a frame on it
    servo_x.write(servo_x_start);
    servo_y.write(servo_y_start);
  }
  
  else if(!huskylens.isLearned()) {//checking if the ball is learned and detected
    servo_x.write(servo_x_start);
    servo_y.write(servo_y_start);
  }
  
  else if(!huskylens.available()) {//checking huskylens returns center of frame (x and y coordinates)
    servo_x.write(servo_x_start);
    servo_y.write(servo_y_start);
  }
